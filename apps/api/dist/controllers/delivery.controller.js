...
exports.updateAttentionStatus = updateAttentionStatus;
...
async function createDelivery(req, res, next) {
    try {
        ...
        const operatorId = req.user?.id || 'seeded-operator-id'; // Fallback if local auth bypass is used
        ...
        const count = await prisma_js_1.prisma.delivery.count({
            where: {
                collectedAt: {
                    gte: todayStart,
                    lte: todayEnd
        ...
        let isDuplicate = await prisma_js_1.prisma.delivery.findUnique({ where: { receiptNumber } });
        while (isDuplicate) {
            sequence++;
            receiptNumber = `HT-${dateStr}-${String(sequence).padStart(4, '0')}`;
            isDuplicate = await prisma_js_1.prisma.delivery.findUnique({ where: { receiptNumber } });
        }
...
async function getDeliveries(req, res, next) {
    try {
        ...
        if (fromDateStr || toDateStr) {
            where.collectedAt = {};
            if (fromDateStr)
                where.collectedAt.gte = new Date(fromDateStr);
            if (toDateStr)
                where.collectedAt.lte = new Date(toDateStr);
        }
        if (memberId)
            where.memberId = memberId;
        if (produceTypeId)
            where.produceTypeId = produceTypeId;
        if (collectionPointId)
            where.collectionPointId = collectionPointId;
        if (paymentStatus)
            where.paymentStatus = paymentStatus;
        if (attentionStatus)
            where.attentionStatus = attentionStatus;
        // We need to count total records matching filters
        const total = await prisma_js_1.prisma.delivery.count({ where });
        ...
        const matchedDeliveries = await prisma_js_1.prisma.delivery.findMany({
            where,
            select: {
                id: true,
...
async function updateDelivery(req, res, next) {
    try {
        const id = req.params.id;
        const data = delivery_validator_js_1.updateDeliverySchema.parse(req.body);
        const userId = req.user?.id || 'seeded-operator-id';
        const existing = await prisma_js_1.prisma.delivery.findUnique({ where: { id } });
        if (!existing) {
            return res.status(404).json({
                success: false,
                error: {
...
async function updateAttentionStatus(req, res, next) {
    try {
        const id = req.params.id;
        const { status, note, neededAttention, reasonCategory } = req.body;
        const userId = req.user?.id || 'seeded-secretary-id';
