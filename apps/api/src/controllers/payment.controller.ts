...
export async function createPayment(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const secretaryId = (req as any).user?.id || 'seeded-secretary-id';
    ...
    const totalDelivered = preciseRound(deliveriesSum._sum.netAmount || 0);
    const totalPaid = preciseRound(paymentsSum._sum.amount || 0);
    const outstandingBefore = preciseRound(totalDelivered - totalPaid);
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const d of openDeliveries) {
        ...
        const amountDue = preciseRound(d.netAmount - alreadyAllocated);

        if (amountDue <= 0) continue;

        const allocateAmount = preciseRound(Math.min(remainingPayment, amountDue));
        remainingPayment = preciseRound(remainingPayment - allocateAmount);
        ...
        const isFullyPaid = preciseRound(alreadyAllocated + allocateAmount) >= d.netAmount;
      ...
      const outstandingAfter = preciseRound(outstandingBefore - data.amount);
